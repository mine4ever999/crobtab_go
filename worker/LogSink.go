...
type LogSink struct {
	...
	logCollection  *mongo.Collection // 表Log的集合
...
func (logSink *LogSink) writeLoop() {
	...
	for {
		...
		case log = <-logSink.logChan:
			// 如果每次插入一条数据 需要等待mongodb的一次往返请求 耗时
			// 为了提高插入效率, 采用批量插入的方式, 一次插入多条(攒批)
			if logBatch == nil {
				// 创建一个新的批次
				logBatch = &common.LogBatch{}
				// 当前批次未满 则无法提交
				// 优化   让这个批次超时自动提交(给1秒的时间）
				commitTimer = time.AfterFunc(
					time.Duration(G_config.JobLogCommitTimeout)*time.Millisecond,
					// writeLoop 和 AfterFunc 是两个协程
					// 需要改成串行处理
					// 闭包函数内的 *common.LogBatch 是传入的logBatch
		...
		// bug  提交超时批次的定时器 与 批次满出发保存同时触发 定时器无法取消
		// 可能会写入两次日志
		// 监听超时批次
